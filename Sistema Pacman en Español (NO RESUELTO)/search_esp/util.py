...
"""
 Estructuras de datos utilies para implementar SearchAgents
"""

class Stack:
    "Un contenedor con politica de 'encolamiento' ultimo-adentro-primero-afuera (LIFO)"

    def __init__(self):
        self.list = []

    def push(self,item):
        "Introduce 'item' a la pila"
        self.list.append(item)

    def pop(self):
        "Extrae el objeto recien metido de la pila"
        return self.list.pop()

    def isEmpty(self):
        "Regresa verdadero si la pila está vacia"
        return len(self.list) == 0

class Queue:
    "Un contenedor con politica de 'encolamiento' primero-adentro-primero-afuera (FIFO)"
    def __init__(self):
        self.list = []

    def push(self,item):
        "Introduce el objeto 'item' en la cola"
        self.list.insert(0,item)

    def pop(self):
        """
          Remueve de la cola el objeto introducido más temprano. Esta
          operación remueve el objeto de la cola.
        """
        return self.list.pop()

    def isEmpty(self):
        "Regresa verdadero si la cola está vacia"
        return len(self.list) == 0

class PriorityQueue:
    """
      Implementa una cola de prioridad. Cada objeto insertado tiene una
      prioridad asociada con este y el cliente usualmente está interesado
      en remover de manera rápida el objeto de menor prioridad dentro de la cola.
      Esta estructura permite un acceso O(1) al objeto de menor prioridad.
    """
    def  __init__(self):
        self.heap = []
        self.count = 0
...
class PriorityQueueWithFunction(PriorityQueue):
    """
    Implementa una cola de prioridad con la misma firma de las clases Queue y Stack
    para los métodos push y pop. Está diseñada para una sustitución rápida de esas
    dos clases. Quien llama la función debe proveer una función de prioridad, la cual
    extrae la prioridad de cada objeto.
    """
    ...
    def push(self, item):
        "Agrega un objeto item a la cola con prioridad de la función de prioridad"
        PriorityQueue.push(self, item, self.priorityFunction(item))


def manhattanDistance( xy1, xy2 ):
    "Regresa la distancia Manhattan entre dos puntos xy1 y xy2"
    return abs( xy1[0] - xy2[0] ) + abs( xy1[1] - xy2[1] )

"""
  Estructuras de datos y funciones útiles para varios proyectos del curso.

  El proyecto de búsqueda no debería necesitar nada de lo que se
  encuentra debajo de esta línea.
"""
...
def raiseNotDefined():
    ...
    print "*** Método no implementado: %s en la linea %s del archivo %s" % (method, line, fileName)
    sys.exit(1)
